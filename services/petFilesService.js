...
class PetListingService {
  ...
  async createPersonalPet(user, data, files) {
  ...
  // ---------------- ADD PET IMAGES ---------------- //
  async addPetImages(user, petId, files) {
  ...
  async getMyListings(user) {
    return Pet.find({ owner: user.id, listingType: 'personal' })
      .sort({ createdAt: -1 })
  ...
  async updatePersonalListing(user, petId, data) {
    const pet = await Pet.findById(petId);
    if (!pet) throw createError('Pet listing not found', 404);
  ...
  async deletePersonalListing(user, petId) {
    const pet = await Pet.findById(petId);
    if (!pet) throw createError('Pet listing not found', 404);
  ...
  // ---------------- DETAILED LISTINGS + ANALYTICS ---------------- //
  async getDetailedListings(user) {
  ...
  // ---------------- FOSTER CHAT ---------------- //
  async startFosterChat(user, petId, requestId) {
    const pet = await Pet.findById(petId)
    ...
    if (fosterRequest.chatThread) {
      ...
      if (existingChat)
        return { existing: true, chat: existingChat, chatThreadId: fosterRequest.chatThread, fosterRequest };
    }
  ...
  // ---------------- FOSTER MEETING ---------------- //
  async scheduleFosterMeeting(user, petId, requestId, meetingData) {
  ...
  // ---------------- GET MY FOSTER REQUESTS ---------------- //
  async getMyFosterRequests(user) {
    ...
    return petsWithUserRequests
      .map(pet => {
        const userRequest = pet.fosterRequests.find(reqItem => reqItem.user && reqItem.user._id.toString() === user.id);
        return userRequest
          ? {
              pet: {
                _id: pet._id,
...
// ---------------- EXPORTS ---------------- //
const petListingService = new PetListingService();
...
module.exports = {
  ...
  // Legacy bindings
  createPersonalPet: petListingService.createPersonalPet.bind(petListingService),
  addPetImages: petListingService.addPetImages.bind(petListingService),
  getMyListings: petListingService.getMyListings.bind(petListingService),
  getAllListings: petListingService.getAllListings.bind(petListingService),
  updatePersonalListing: petListingService.updatePersonalListing.bind(petListingService),
  deletePersonalListing: petListingService.deletePersonalListing.bind(petListingService),
  getDetailedListings: petListingService.getDetailedListings.bind(petListingService),
  startFosterChat: petListingService.startFosterChat.bind(petListingService),
  scheduleFosterMeeting: petListingService.scheduleFosterMeeting.bind(petListingService),
  getMyFosterRequests: petListingService.getMyFosterRequests.bind(petListingService),
};
