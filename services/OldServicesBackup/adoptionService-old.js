...
const { NotificationService } = require('../notificationService'); // ✅ Replace Notifier
...
exports.getMyRequests = async (userId) => {
  ...
  // Filter out requests where pet is null (soft-deleted or not found)
...
exports.requestAdoption = async (user, petId) => {
  ...
  // ✅ FIX: Extract proper IDs
  await NotificationService.create({
    users: staffUsers.map(staff => staff._id.toString()), // Convert to string
    type: NOTIFICATION_TYPES.ADOPTION_REQUEST_NEW,
    message: `New adoption request for ${pet.name}`,
    meta: { 
      petId: pet._id.toString(), 
      adopterId: user.id.toString(), 
...
exports.updateRequestStatus = async (user, requestId, body) => {
  ...
  if (['ignored', 'rejected'].includes(status)) {
    ...
    // ✅ REPLACE: Notifier.notifyUser → NotificationService.create
    await NotificationService.create({
...
exports.confirmMeeting = async (user, requestId, notes) => {
  ...
  // ✅ REPLACE: Notifier.notifyUsers → NotificationService.create
  await NotificationService.create({
    users: staffUsers.map(staff => staff._id),
